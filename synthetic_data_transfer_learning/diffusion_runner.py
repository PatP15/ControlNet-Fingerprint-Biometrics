...
DATA_FOLDER  = '/data/puma_envs/img_l2_feature_extractions/enhance_control_generated'

print('prints gan data pretraining')

def main():
    parser = argparse.ArgumentParser(description='Pretraining on PrinstGAN')

    parser.add_argument('--model_path', type=str, default='/data/puma_envs/FingerPrintMatchingModels/diffusion/model_embeddings_diffusion2.pth',
                        help='Where to save the model at the end')
    parser.add_argument('--data_path', type=str, default=DATA_FOLDER,
                        help='Where the PrintsGAN data is')
    parser.add_argument('--output_folder', type=str, default='/data/puma_envs/FingerPrintMatchingModels/diffusion/diffusion_pretrain_results2',
    ...
    batch_size=64
    #0.8, 0.1, 0.1 split
    training_dataset = TripletDataset(PrintsGANDataset(os.path.join(args.data_path, 'train'), 50000, 3, train=True))
    ...
    val_dataset = TripletDataset(PrintsGANDataset(os.path.join(args.data_path, 'val'), 10000, 3, train=False))
    ...
    best_val_epoch, best_val_loss, all_epochs, train_losses, val_losses = fit(
        ...
        n_epochs=n_epochs, cuda='cuda', log_interval=1000, metrics=[], start_epoch=0, early_stopping_interval=15
    )
